...
\usepackage{zi4} % nicer typewriter font, optional
\usepackage{tikz}
\usetikzlibrary{positioning,fit,backgrounds,calc,patterns}
...
\begin{centering}
\large 
For the Photoemission Module:\\
\vspace{4mm}
Felix Mildner, Victor Chang Lee, Bruno Camino, Nicholas M. Harrison\\
\vspace{4mm}
Department of Materials\\
...
\vspace{5mm}
\copyright\ 2013-2026 A.~J. Morris, R.~J. Nicholls, C.~J. Pickard and J.~R. Yates.\\
\vspace{5mm}
...
\item[{\bf --}]  \verb#One step photoemission model# : The one step model has no final band in the crystal. The final state is a free-electron state in the vacuum continuum,
...
where $\lambda(\omega)$ is the electrons scattering length and $sin (1-\theta (i,j,\bf{k}))$ considers the scattering length increase with emission angles away from the surface normal. How the matrix element to that continuum is formed is described in Section \ref{subsec:coherency_tensor}.
\end{itemize}

\subsection{The free-electron matrix element and the coherency tensor}
\label{subsec:coherency_tensor}

The final state of Equation \eqref{1_final_electron_wave} is not a single plane
wave: at a given final energy $E_f$ every reciprocal lattice vector $\mathbf{G}$
with $\left|\mathbf{k}+\mathbf{G}\right|^2/2 \approx E_f$ supplies an available
free-electron state. These states are mutually orthogonal, so Fermi's golden rule
adds their \emph{rates}, not their amplitudes, and the sum over them is what
supplies the final-state density of states $\rho_f$.

CASTEP evaluates this as a coherency tensor, written to a {\tt *.fem\_bin} file
for each state and binned in final-state energy,

\begin{equation}
A_{ab}(n,\mathbf{k},E_f) = \sum_{\mathbf{G}} M^{\mathbf{G}}_{a} \left( M^{\mathbf{G}}_{b} \right)^{*}
                           ~g\!\left(E_f - E_{\mathbf{k}+\mathbf{G}}\right)
    \label{eq:coherency_tensor}
\end{equation}

where $M^{\mathbf{G}}_{a}$ is the plane wave coefficient of
$v_a \left| n,\mathbf{k} \right\rangle$ for Cartesian direction $a$, $v_a$ is the
velocity operator including its non-local and, where present, Hubbard and
meta-GGA contributions, $E_{\mathbf{k}+\mathbf{G}} = \left|\mathbf{k}+\mathbf{G}\right|^2/2$
and $g$ is a normalised Gaussian of width {\tt fem\_broadening}. $A_{ab}$ is
Hermitian in $(a,b)$ and is stored as its six independent components in the order
$(11,22,33,12,13,23)$.

Because the sum in Equation \eqref{eq:coherency_tensor} is a sum of rates over
orthogonal final states, the result does not depend on the cell volume. This is
the property that distinguishes it from a coherent construction, in which the
plane waves at a given energy are first summed into one wavepacket and the
modulus of that sum is squared. A coherent sum gives a matrix element whose value
depends on the relative phases of coefficients that describe physically distinct
final states, and consequently on the size of the simulation cell through the
density of $\mathbf{G}$ vectors.

The photoemission matrix element for a light polarisation direction
$\mathbf{\hat{q}}$ is recovered from the tensor by contraction,

\begin{equation}
M(i,\mathbf{k},s) = \frac{1}{(\hbar\omega)^2}\,\hat{q}_a \hat{q}_b\, A_{ab}(i,\mathbf{k},E_f)
    \label{eq:fem_contract}
\end{equation}

with the imaginary parts of the off-diagonal components cancelling in pairs for
real $\mathbf{\hat{q}}$. For {\tt optics\_geom : unpolar} the contraction is
performed for the two directions perpendicular to {\tt optics\_qdir} and
averaged, and where the cell has symmetry operations the polarisation direction
is expanded over them first, exactly as for the optical matrix elements of
Section \ref{subsec:opticprops}.

The energy at which the tensor is sampled is

\begin{equation}
E_f = E(i,\mathbf{k},s) + \hbar\omega - E_{vac}
    \label{eq:fem_lookup}
\end{equation}

referenced to the vacuum level rather than to the eigenvalue zero, and linearly
interpolated between the two neighbouring bins. The reference matters. The
tensor is binned by $\left|\mathbf{k}+\mathbf{G}\right|^2/2$, whose zero is the
cell-averaged local potential, and that average shifts as vacuum is added to a
slab -- by $2.7$ eV over a $10$ to $28$ \AA\ vacuum scan on an otherwise
identical slab. A photoelectron travels in the vacuum, not in the average
potential, so referencing the lookup to $E_{vac}$ is what makes the result
independent of the amount of vacuum in the cell: across converged cells the
spread in the computed quantity falls from $1.264$ eV to $0.070$ eV, and what
remains is the work-function spread, which is physical.

Requesting a final-state energy outside the stored window returns zero. Below the
window there are no free-electron final states at all; above it the occurrence is
counted and reported, since it means the window written by CASTEP was too narrow
for the photon energy being used.
...
\subsection{Layers, boxes and the slab boundaries}
\label{subsec:layer_boxes}

The slab is divided into boxes, one per atomic layer, and most of what follows is
computed per box: a dielectric function from the states projected onto its atoms,
an absorption coefficient, an escape depth. The division is a single geometry
however it is supplied.

The slab runs from $z_{top}$ down to $z_{middle}$ and is cut by $N+1$ planes,

\begin{equation}
z_{top} = b_0 > b_1 > \dots > b_N = z_{middle}
\end{equation}

so box $i$ occupies $[b_i, b_{i-1}]$ and has volume $(b_{i-1}-b_i)A$ with $A$ the
in-plane cell area. The boxes tile the slab exactly, so their volumes add up to
it, and every atom belongs to one of them. Everything below $z_{middle}$ is
covered by the bulk extrapolation of Section~\ref{subsec:momentum_outputs}.

$z_{top}$ is {\tt photo\_slab\_max} and $z_{middle}$ follows from
{\tt photo\_slab\_middle}, or from the midpoint of the slab bounds if that is not
given. The interior planes $b_1 \dots b_{N-1}$ are {\tt photo\_layers\_tops} when
those are supplied. Otherwise they are inferred: the atoms are clustered into
layers by their spacing in $z$, and each boundary is placed midway between
neighbouring layer centroids. That reduces to a box per layer of the bulk
interlayer spacing when the slab is evenly spaced, and still tiles when it is
not.

Three lengths follow from this and they are not the same:

\begin{itemize}
\item the \emph{box height} $b_{i-1}-b_i$, which sets the volume that normalises
      that box's dielectric function;
\item the \emph{layer gap} $c_i - c_{i+1}$ between the centroids of neighbouring
      layers, which is the distance the light travels from the atoms of one layer
      to those of the next;
\item the \emph{bulk repeat}, the interlayer spacing of the material below the
      explicit region, which is what the bulk extrapolation steps by.
\end{itemize}

They coincide only for an evenly spaced slab whose boundaries fall midway between
its layers, which is why they are computed separately.

\paragraph{What the inference will not do.} The layers are only meaningful as
equivalent repeats of one material: the explicit region is chosen by depth
alone, each box gets one dielectric function, the topmost box's optical constants
stand for the whole surface, and the deepest explicit box is repeated downwards
as though the substrate were made of it. An interface, an adsorbate, or a
compound whose planes alternate in species breaks all four. OptaDOS therefore
checks that the inferred layers all hold the same species and stops if they do
not, printing the composition it found. Differing numbers of atoms per layer, and
layer spacings that vary, are reported as warnings rather than errors. In each
case the remedy is {\tt photo\_layers\_tops}, which says exactly what is wanted
and allows a full repeat unit of a material to be one layer rather than one plane
of one species.
...
\begin{equation}
I(\omega,\mu) = I(\omega, \mu-1)\,e^{-\frac{1}{2}[\alpha(\omega,\mu-1)+\alpha(\omega,\mu)]\,d(\mu-1,\mu)}
    \label{Intensity}
\end{equation}

with $I(\omega,1) = I_0 - R(\omega,1)$, where $\alpha(\omega,\mu)$ is the absorption coefficient of layer $\mu$, $d(\mu-1,\mu)$ is the layer gap of Section~\ref{subsec:layer_boxes} and $R$ is the reflectivity. The step from one layer to the next crosses part of each of the two boxes, since the boundary between them lies inside the gap, so the exponent takes the mean of the two absorption coefficients over that one distance. Below the explicit region every step is through another repeat of the same material, so a single coefficient and the bulk repeat spacing are used instead. The calculation of the optical characteristics is analogous to the equations in Section \ref{subsec:opticprops}, but the contributions from each layer is calculated by weighting the electronic structure by the weights $W$ from Section \ref{subsec:opticprops}.
...
\begin{equation}
\begin{split}
\theta_{int}{(i,j, {\bf{k}},s)} &= \arccos \sqrt{\frac{E_K(j,{\bf{k}},s)+V_0-E_\parallel(i,{\bf{k}},s)}{E_K(j,{\bf{k}},s)+V_0}}\\
\theta_{ext}{(i,j, {\bf{k}},s)} &= \arccos \sqrt{\frac{E_K(j,{\bf{k}},s)-E_\parallel(i,{\bf{k}},s)}{E_K(j,{\bf{k}},s)}}
\end{split}
\label{theta}
\end{equation}

where $E_\parallel(i,\bf{k})$ is the transverse energy, $E_K(j,\bf{k})$ is the final state kinetic energy and $V_0$ is the inner potential, set by {\tt photo\_inner\_potential}. The angle $\theta_{ext}$ is used to determine if emission is possible under the chosen conditions, while $\theta_{int}$ is used to model the electron's propagation distance.

The numerator is the energy along the surface normal, $E_K - E_\parallel$, and the square root is not optional. $\theta$ is defined by a ratio of momenta, $\cos\theta = k_z/|k|$, and converting each momentum into an energy squares it, so the bare energy ratio is $\cos^2\theta$ rather than $\cos\theta$. Both limits agree either way -- normal emission gives a ratio of one and grazing emission a ratio of zero -- so the difference is invisible at the ends of the range and largest in the middle, reaching $15.8^\circ$ at $E_\parallel/E_K = 2/3$.

\subsection{The inner potential}
\label{subsec:inner_potential}

$V_0$ is the depth of the well the photoelectron climbs out of, measured from the
vacuum level down to the bottom of the free-electron-like final state band. It is
not the work function, which is measured from the Fermi level: for Cu the two are
roughly 4.3 and 13.5 eV, so taking the work function for the barrier understates
the refraction by about a factor of three.

There is no safe way to derive it automatically, which is why it is a keyword.
Two estimates are worth having, and both are printed by {\tt workfct.py} from the
same planar-averaged {\tt .pot\_fmt} already used for the work function:

\begin{itemize}
\item $E_{vac} - \min(\varepsilon_{n\bf{k}})$, about 13.5 eV for these slabs and
      in line with the 13--14 eV usually fitted for Cu. It is right only when the
      lowest computed band happens to be the free-electron-like one, which is a
      property of the valence window of the pseudopotential rather than of the
      material -- a Cu potential carrying 3s and 3p as semicore moves it by 70 eV
      or more.
\item $E_{vac} - \langle V_{loc}\rangle_{bulk}$, from the \emph{macroscopic}
      average of the planar-averaged local potential: a convolution with a
      normalised box one lattice period wide along $z$, which removes the
      oscillation with the atomic planes and leaves a plateau wherever the
      material is bulk-like. Well defined and quick to converge with slab
      thickness, but it is the mean local potential, which lies below the band
      bottom by the zero-point kinetic energy of the Bloch state and by the
      non-local part of the pseudopotential, so it overestimates $V_0$.
\end{itemize}

In practice $V_0$ is fitted from photon-energy-dependent normal-emission
measurements. Use the band estimate as a starting value where the valence window
is known to be clean, use the flatness of the macroscopic average as the
diagnostic that the slab has a bulk-like middle at all, and state the value
chosen. The answer is not very sensitive once $V_0$ is in the right range: on a
five-layer Cu slab at 6 eV the quantum efficiency moves by 3.0\% on going from
the work function to 13.5 eV, and only 0.6\% further on going to 22.8 eV.
...
\begin{equation}
    T(E_f,\mathbf{k_{||}}) = \frac{\sum_{(k+G)_\perp>0}\left|u_f(\mathbf{G},\mathbf{k})\right|^2}
                                  {\sum_{\mathbf{G}}\left|u_f(\mathbf{G},\mathbf{k})\right|^2}
    \label{eq:transmit_prob}
\end{equation}

Two features of Equation \eqref{eq:transmit_prob} deserve comment.

The numerator sums the squared moduli of the coefficients rather than squaring
their sum. The plane waves are orthogonal final states, so the electron reaches
the vacuum in one or another of them and the probabilities add; forming
$\left|\sum_\mathbf{G} u_f\right|^2$ instead would add the amplitudes coherently
and make the result depend on the arbitrary phase convention of each
coefficient.

The denominator normalises by the total plane-wave weight of the same state. It
is written this way because with an ultrasoft pseudopotential the wavefunction is
normalised as $\left\langle \phi \left| S \right| \phi \right\rangle = 1$ rather
than $\left\langle \phi \mid \phi \right\rangle = 1$, so the augmentation charge
carried by the projectors is missing from the plane-wave part and
$\sum_\mathbf{G}\left|u_f\right|^2$ falls below unity. The shortfall is large and
strongly state dependent: for Cu it is around $0.30$ for the localised $3d$
states against $0.95$ for free-electron-like states. Without the denominator the
quantity would therefore report the pseudopotential's augmentation charge as
much as the surface physics, and $d$-derived bands would be suppressed by a
factor of three relative to $sp$-derived ones for a reason that has nothing to do
with transmission. Since $T$ is a probability it cannot depend on the choice of
pseudopotential, and dividing by the total restores that. For a norm-conserving
pseudopotential the denominator is unity and the normalisation does nothing.
...
This expression has no floor, so a large enough field drives $W_{eff}$ to zero
and below. That is the field emission regime, where electrons leave without a
photon and the three-step model no longer describes what happens; it is also
where several of the guards in the code stop holding, because they assume the
vacuum level lies above the Fermi level. In particular, when
{\tt photo\_inner\_potential} is not set the surface barrier $V_0$ of
equation~(\ref{theta}) falls back to $W_{eff}$, and a negative $W_{eff}$ makes
$\theta_{int}$ the arc cosine of the square root of a negative number. The
resulting {\tt NaN} compares false against every subsequent test, so those states
are dropped without a word and the calculation still returns a number. OptaDOS
therefore stops when $W_{eff} \leq 0$, reporting the work function, the lowering
and their difference. The limit is
\begin{equation}
F < \frac{4 \pi \varepsilon_0 W^2}{e^3} \approx \left(\frac{W [\mathrm{eV}]}{3.79 \times 10^{-5}}\right)^2 \frac{\mathrm{V}}{\mathrm{m}}
\end{equation}
which is about $1.3 \times 10^{10}$ \nicefrac{V}{m} for a work function of
4.3~eV. For scale, a photocathode gun runs at $10^{7}$ to $10^{8}$ \nicefrac{V}{m}
and a field emitter tip at a few times $10^{9}$, so the limit is only reached by
a deliberate choice or by a slip of units.
...
\begin{equation}
    G \equiv g_e \int_{z_1}^{z_2} M^{\frac{1}{2}}dz~~\text{with}~~g_e=\frac{2(2 m_e)^{1/2}}{\hbar}
\end{equation}
where the integral is over the range with $M>0$, i.e. $z_1$ and $z_2$, $m_e$ is the electron mass and $\hbar$ is reduced Planck's constant.\cite{Forbes2007} The integral boundaries are calculated as turning points of M by using
\begin{equation}
z_{2,1} = \frac{\phi \pm \sqrt{\phi^2-\frac{e^3F}{4\pi\varepsilon_0}}}{2eF}
\end{equation}
where $\phi$ is the barrier height of the electron in $J$, $e$ is the electron charge in $C$, $F$ is the electric field in $V/m$, and $\varepsilon_0$ is the vacuum permittivity in . The numerical integration for each band's is performed using a recursive bisection method, where each segment's contribution is approximated using Simpson's rule.
...
\end{align*}
The values of $\alpha_{max}$ and $\beta_{max}$ are chosen, so that $|c_x| < 3.8 $ \nicefrac{1}{\AA} and  $|c_y| < 3.8 $ \nicefrac{1}{\AA} to limit the number of grid points. The value on each of the grid points is calculated by summing the squared plane wave coefficients over the $z$ direction and normalising by the total plane-wave weight of the state,
\begin{equation}
    w_{gk}(\mathbf{k_{x,y} + G_{x,y}}) = \frac{\sum_{G_z} \left| u_f(\mathbf{G},\mathbf{k_{x,y}}) \right|^2}
                                              {\sum_{\mathbf{G}} \left| u_f(\mathbf{G},\mathbf{k}) \right|^2}
    \label{eq:gkgrid_weight}
\end{equation}
where $u_f$ are the plane wave basis coefficients. The denominator is there for the reason given below Equation \eqref{eq:transmit_prob}: with an ultrasoft pseudopotential the plane-wave part of the state does not carry unit norm, so without it $w_{gk}$ would inherit the augmentation charge and weight localised states differently from free-electron-like ones for a purely technical reason. The weights then sum to one over the retained grid, up to whatever falls outside the $3.8$ \nicefrac{1}{\AA} cutoff; CASTEP reports the retained fraction at {\tt iprint} $\geq 2$, and in practice it is above $0.98$.

The $c_{x,y}$ and $w_{gk}$ values are read from a {\tt *.gkgrid\_bin} file. The calculation of QE and MTE values is performed analogous to Section \ref{subsec:QE_MTE} for each of the grid points with $c_{x,y}$ replacing $\mathbf{k_{||}}$, and $w_{gk}$ weighting each grid point's contribution.

Note that the $\mathbf{k+G}$ set generated for one irreducible $\mathbf{k}$ point is not symmetric about the origin -- an irreducible wedge cannot be. Any quantity that must respect the crystal point group, such as the outputs of Section \ref{subsec:momentum_outputs}, is therefore expanded over the symmetry operations of the cell before being binned.

\subsection{Momentum resolved outputs}
\label{subsec:momentum_outputs}

Besides the integrated quantum efficiency and mean transverse energy, the module
can write the emission resolved in energy and momentum: a kinetic energy against
transverse momentum map, a $p_x$ against $p_y$ map at one binding energy, and the
full $p_x,p_y,p_z$ tensor. These are selected with {\tt photo\_output} and all of
them work with either setting of {\tt photo\_momentum}. Three conventions apply
to all of them.

\paragraph{Energy is assigned to the initial band.} Every emission quantity is
indexed by the initial band with the photon energy folded in,

\begin{equation}
E_K(i,\mathbf{k},s) = E(i,\mathbf{k},s) + \hbar\omega - E_{vac}
    \label{eq:map_indexing}
\end{equation}

so $E_K$ is the kinetic energy of a photoelectron that started in band $i$,
rather than the kinetic energy of band $i$. In the one step model this is exact,
since there is no final band to index by. In the three step model there is a
discrete final band, and $E(j,\mathbf{k},s) - E_{vac}$ would be the exact kinetic
energy of the emitted electron; Equation \eqref{eq:map_indexing} is used instead
because its error is bounded by a resolution limit the model already has. Energy
conservation is enforced by the broadened delta function of Equation
\eqref{fermi_golden_rule}, so for any pair that contributes,
$E(j) = E(i) + \hbar\omega$ holds only to within the broadening width -- of order
$0.1$ to $0.5$ eV for the adaptive scheme. Assigning the exact final energy and
then slicing it with a much narrower Gaussian would manufacture structure the
approximation cannot support, whereas Equation \eqref{eq:map_indexing} places
each pair at the energy the delta function is centred on. It also keeps the two
models directly comparable.

A consequence worth knowing is that the effective energy resolution of these
outputs is the delta-function width, not {\tt photo\_bindenergy\_broadening}.

\paragraph{The momentum grids are centred on zero.} A $p_x$ or $p_y$ axis of
$2n+1$ bins runs from $-n$ to $+n$ in units of {\tt photo\_pmat\_bin\_width},
with $p = 0$ on the central bin. The $p_z$ axis of the tensor starts at the value
printed in the file header and increases by one bin width per index. Reading a
map therefore only requires the shape and the bin width, both of which are in the
header.

\paragraph{The transverse distribution is symmetrised.} Contributions are
expanded over the crystal symmetry operations before binning, so an output
reproduces the point group of the surface even though the $\mathbf{k}$ points, or
the $\mathbf{k+G}$ sets of Equation \eqref{eq:gkgrid_weight}, cover only an
irreducible wedge. For a surface of $4mm$ symmetry, for instance, the two
transverse marginals of the tensor are even and equal to each other, and the four
quadrants carry equal weight.

The extent of the transverse grid is set by the kinematics rather than by the
cell: no emitted electron can carry more transverse momentum than
$\sqrt{2m_e(\hbar\omega - W)}/\hbar$, since the remainder of its energy is needed
for the motion along the surface normal, and the grid is sized to that bound with
a small margin. It follows that the grid does not grow when a supercell is used,
and that plane wave components lying outside it are components that cannot emit.

Note that these outputs are written as formatted text and become large: the
tensor is one number per grid point, which for a typical slab is a few hundred
megabytes per photon energy. A {\tt photo\_energy\_sweep} writes one file per
step.

\subsection{Angular acceptance}
\label{subsec:angular_acceptance}

All of the outputs above may be restricted to an angular window, standing in for
the acceptance of an analyser. Both angles are given the same way, as a direction
and a half width in degrees: {\tt photo\_theta\_centre} and
{\tt photo\_theta\_halfwidth} for the polar angle, measured from the surface
normal, and {\tt photo\_phi\_centre} and {\tt photo\_phi\_halfwidth} for the
azimuth, measured from the cartesian $x$ axis.

A pair of bounds cannot describe an analyser centred on the $x$ axis, which spans
$-30^\circ$ to $+30^\circ$ and wraps through $180^\circ$; a centre and a half
width can. The defaults, $45^\circ \pm 45^\circ$ in $\theta$ and
$0^\circ \pm 180^\circ$ in $\phi$, accept everything.

The two differ in that the azimuth wraps and the polar angle does not. $\theta$
is measured from the surface normal and cannot physically exceed $90^\circ$, so
its window is clipped to $0^\circ$--$90^\circ$ when it is applied rather than
allowed to run past. That clip matters: states that cannot emit under the chosen
conditions are marked with a $\theta$ of $91^\circ$, and an unclipped window
could reach past $90^\circ$ and admit them.

The earlier keywords {\tt photo\_theta\_min}, {\tt photo\_theta\_max},
{\tt photo\_phi\_min} and {\tt photo\_phi\_max} are no longer accepted. For
$\theta$ this is only a change of spelling. For $\phi$ it is not: those keywords
acted on an azimuth that had been folded into the first quadrant, so a narrowed
window selected one wedge in each of the four quadrants rather than a single
wedge, and reinterpreting an old input file against the signed azimuth would
silently change what it asked for.

The two windows are applied at different points, because they behave differently
under the symmetry of the cell. The polar angle depends only on the magnitude of
the transverse momentum, which the in-plane symmetry operations leave unchanged,
so testing it once per irreducible $\mathbf{k}$ point is exact. The azimuth is
not invariant: an irreducible point stands for its whole symmetry star, and that
star spans as many azimuths as the point group has in-plane operations -- eight,
for a $C_{4v}$ surface. The azimuthal window is therefore tested against each
symmetry image separately, at the point where that image's transverse momentum is
known. For the outputs that carry no azimuth axis and so expand no star of their
own -- the binding energy curve, and the maps whose momentum axis is
$|\mathbf{k}_\perp|$ -- the contribution is instead weighted by the fraction of
the star that falls inside the window. For a structure with no symmetry the two
treatments coincide.
...
% =========================================================
% Panel sizes and vertical placement
% =========================================================
% All panels use local coordinates:
% width  = 14
% heights:
%   Functional = 5.0
%   Structural = 3.6
...
\begin{scope}[shift={(0,9.2)}]
  \draw[panel] (0,0) rectangle (14,5.0);
  ...
  \node[module, minimum width=3.0cm] (jdosutils) at (10.6,0.9) {jdos\_utils};

  \draw[dep] (optados.south west) -- (dos.north);
  \draw[dep] (optados.south) -- (pdos.north);
  \draw[dep] (optados.south) -- (core.north);
  \draw[dep] (optados.south) -- (jdos.north);
  \draw[dep] (optados.south east) -- (optics.north);
  \draw[dep] (optados.south) -- (photo.north);

  \draw[dep] (dos.south) -- (dosutils.north west);
  \draw[dep] (pdos.south) -- (dosutils.north);
  \draw[dep] (core.south) -- (dosutils.north);
  \draw[dep] (photo.south) -- (dosutils.north east);

  \draw[dep] (jdos.south) -- (jdosutils.north);
  \draw[dep] (optics.south) -- (jdosutils.north east);
  \draw[dep] (photo.south) -- (jdosutils.north west);

  \draw[dep] (dosutils.east) -- (jdosutils.west);
\end{scope}
...
\begin{scope}[shift={(0,5.3)}]
  \draw[panel] (0,0) rectangle (14,3.6);
  ...
  \node[module, minimum width=2.8cm] (cell)       at (11.8,0.6) {cell};

  \draw[dep] (electronic.south east) -- (parameters.north west);
  \draw[dep] (parameters.south east) -- (cell.north west);
\end{scope}
...
\begin{scope}[shift={(0,3)}]
  \draw[panel] (0,0) rectangle (14,2.0);
  \node[sectiontitle] at (13.7,1.75) {Utility};

  \node[module, minimum width=2.9cm] (algorithms)    at (4.2,0.8) {algorithms};
  \node[module, minimum width=3.9cm] (xmgraceutils)  at (8.6,0.8) {xmgrace\_utils};
\end{scope}
...
\begin{scope}[shift={(0,0)}]
  \draw[panel] (0,0) rectangle (14,2.8);
  ...
  \node[module, minimum width=2.9cm] (constants) at (11.8,0.6) {constants};

  \draw[dep] (io.east) -- (comms.west);
  \draw[dep] (comms.east) -- (constants.west);
\end{scope}
\end{tikzpicture}
\caption{Schematic structure of the program}
...
\item {\bf extra calculations}: A number of options are available.
    ...
    \item {\bf kinetic\_energy\_momentum\_map}: Calculate and write a {\bf kinetic energy vs transverse momentum} map as a matrix with the value of each grid point representing the QE contribution at that point. The contributions are Gaussian broadened and the ranges for emission angles $\theta$ and $\phi$ can be set to allow for comparison with ARPES data.
    ...
    \item {\bf full\_momentum\_tensor}: Calculate and write a {\boldmath$p_x$,$p_y$,$p_z$} tensor of grid points for electron momenta in $\frac{1}{\text{\AA}}$. Each value at that grid point represents the QE contribution at that specific set of electron momenta. The contributions are Gaussian broadened and the ranges for emission angles $\theta$ and $\phi$ can be set to allow for comparison with ARPES data.
    \end{itemize}
    Each of these three has a {\tt \_gkgrid} counterpart, called instead when
    {\tt photo\_momentum : gkgrid}, which bins the transverse momentum over the
    $\mathbf{k+G}$ grid rather than over the crystal $\mathbf{k}$ points. The
    conventions common to all six are described in Section
    \ref{subsec:momentum_outputs}.

\item {\bf read\_transmit\_probabilities}: reads the surface transmission
    probability of Equation \eqref{eq:transmit_prob} from a {\tt *.tmprob\_bin}
    file, when {\tt photo\_use\_tmprob} is set. The probability is indexed by the
    final band, since it is the final state that crosses the surface.

\item {\bf elec\_read\_gk\_grid}: reads the $\mathbf{k+G}$ grid and its weights,
    Equation \eqref{eq:gkgrid_weight}, from a {\tt *.gkgrid\_bin} file when
    {\tt photo\_momentum : gkgrid}. Each of the momentum resolved routines frees
    the grid on exit and re-reads it when called, so the order in which they run
    does not matter.
\end{itemize}
...
\item[{\bf --}]  \verb#photo_energy_sweep#
\item[{\bf --}]  \verb#all#
\end{itemize}
...
When setting the task to \verb#photoemission#, a photoemission calculation for a single photon energy is performed.
By choosing \verb#photo_energy_sweep# as range of photon energies can be defined to do a series of photoemission calculations at different photon energies.
...
Value of the scissor operator, a rigid shift of the conduction manifold applied
to the transition energies in the JDOS and the optical response, to compensate a
DFT band gap that is too small.

{\bf It is not accepted with {\tt task : photoemission} and stops the
calculation.}  The three-step model emits into the free-electron-like states at
and just above the vacuum level, and the only handle the code has on
``conduction'' is ``above the Fermi level'', which catches those final states as
well as the bound conduction bands.  They carry no gap error, so shifting them
misaligns the two halves of the model: the joint density of states then selects a
final band a scissor {\em below} the vacuum level while the escape probability is
computed for an electron a scissor {\em above} it.  Near threshold, where the
whole quantum efficiency sits, that asks for states a gapped material does not
have, and the emission comes out suppressed by an amount set by the size of the
DFT gap error rather than by any physics.  The failure is silent -- a plausible,
smaller number -- which is why it is refused rather than warned about.

The line cannot be drawn any better inside the model.  Separating the bound
conduction bands from the continuum requires the vacuum level, which is placed at
{\tt photo\_work\_function} above the Fermi level, which for a gapped system is
itself determined by the gap.

Correct the gap upstream instead.  A hybrid or meta-GGA functional, DFT+U, or GW
eigenvalues written into the {\tt .bands} file all work: the photoemission task
reads the energies from that file and does not care how they were produced, and
correcting them there fixes the final states as well as the gap.
...
Calculate a DOS dependent emission model based on an extension of the Dowell-Schmerge Model, calculates both individual band contributions and full DOS contributions. Adapted from the model published by Saha, et al.. \cite{Saha2024}
\end{itemize}
...
Sets the base of the slab, $z_{bottom}$, in \AA. Together with {\tt photo\_slab\_max} it brackets the material. It fixes the default position of the explicit/bulk boundary, at the midpoint of the two, and for a slab that is a single atomic layer it is the base of the one box. This needs to be determined by the user; the fall-off of the planar-averaged electron density is the usual criterion, and {\tt workfct.py} in the analysis scripts computes that average.

There is no default value

\subsection[photo\_slab\_max]{\tt real(kind=dp) :: photo\_slab\_max}
Sets the surface of the slab, $z_{top}$, in \AA. The outermost box extends up to it, so a surface taken from the density fall-off gives that box the larger extent it physically has rather than half an interlayer spacing. It is also the top of the confinement length used for the broadening of the photoemission JDOS. Determined by the user in the same way as {\tt photo\_slab\_min}.

When {\tt photo\_layers\_tops} is given, this is taken from its first entry and must not be set separately.

There is no default value

\subsection[photo\_slab\_middle]{\tt real(kind=dp) :: photo\_slab\_middle}
Sets where the explicitly treated region ends and the bulk extrapolation begins, in \AA. Every layer whose centroid lies above it is treated explicitly.

Only the choice of layer is taken from this value: the boundary itself is then placed midway between the last explicit layer and the first bulk one, like every other boundary, so the exact number does not leak into that box's volume or its optical constants. Any value selecting the same layer gives the same answer.

It may be given with {\tt photo\_slab\_min} and {\tt photo\_slab\_max}, and must be given with {\tt photo\_layers\_tops}. Left unset with slab bounds it defaults to the midpoint of the two, which keeps roughly the top half of the layers.

The default is the midpoint of {\tt photo\_slab\_min} and {\tt photo\_slab\_max}

\subsection[photo\_layers\_tops]{\tt real(kind=dp) :: photo\_layers\_tops(\#layers)}

This is a vector of values on a single line, separated by spaces --- not a
block. It sets the top $z$-coordinate of each layer in \AA, going from the
surface towards the middle:

\begin{verbatim}
photo_layers_tops : 31.5503 28.8781 27.0966 25.3150 23.5335 21.7520 19.9713
photo_slab_middle : 18.1890
\end{verbatim}

The first entry is the slab surface and therefore fixes {\tt
photo\_slab\_max}, which must not then be set separately; the remaining entries
are the boundaries between the boxes, and {\tt photo\_slab\_middle} closes the
last one. The number of entries is the number of explicitly treated boxes.

Giving them explicitly overrides the boundaries OptaDOS would otherwise infer, and is the way to describe anything the inference cannot: an interface, an adsorbate, or a compound whose planes alternate in species. OptaDOS refuses those rather than guessing --- see Section~\ref{subsec:layer_boxes}.
...
\subsection[photo\_inner\_potential]{\tt real(kind=dp) :: photo\_inner\_potential}

Sets the inner potential $V_0$, the depth of the well below the vacuum level, in
eV. This is the barrier an escaping electron climbs, and it is what sets the
refraction at the surface through $\theta_{int}$ in
Equation~\ref{theta}. See Section~\ref{subsec:inner_potential}.

It is not the work function. The work function is measured from the Fermi level
and the inner potential from the bottom of the free-electron-like final state
band; for Cu they are about 4.3 and 13.5 eV. Using the work function bends the
electron too little, leaves $\theta_{int}$ too large and makes the escape path
$1/\cos\theta_{int}$ too long.

Left unset, the code falls back to {\tt photo\_work\_function} and says so in the
output. That is what earlier versions always did, so no existing input changes
its answer, but the fallback is known to be too small.
...
Sets the external electric field in \nicefrac{V}{m}, applied normal to the
surface. The field enters the calculation in two places. It lowers the barrier by
the Schottky term of section~\ref{subsec:fieldemission}, so the effective work
function $W_{eff}$ and hence the vacuum level used throughout the emission step
both move; and it sets the Schottky-Nordheim barrier whose transmission
probability multiplies the escaping current.

Note the unit. A field quoted in \nicefrac{V}{nm} or \nicefrac{V}{\AA}, as
field emission work usually is, must be converted: 1 \nicefrac{V}{nm} is
$10^{9}$ \nicefrac{V}{m}.

The field may not be large enough to close the barrier. OptaDOS stops if the
Schottky lowering reaches the work function, which for a work function of 4.3~eV
happens at about $1.3 \times 10^{10}$ \nicefrac{V}{m}; see
section~\ref{subsec:fieldemission} for the limit and why it is enforced rather
than clamped.

The default value is 0.0 \nicefrac{V}{m}

\subsection[photo\_imfp\_value]{\tt character(len=20) :: photo\_imfp\_model}
\begin{itemize}
    \item[{\bf --}]  \verb#const# (default)
    \item[{\bf --}]  \verb#layers#
    \item[{\bf --}]  \verb#cu_curve# (experimental)
\end{itemize}
This sets the model for how to treat the {\tt photo\_imfp\_value} input. For the {\tt const} option a single value is used for all layers in the slab. For the {\tt layers} option each explicit layer must be given a value. The IMFP value for a specific layer is calculated as a weighted average of all the above layers. The {\tt curve} option is only experimental and calculates an IMFP value based on each electron's excess energy during propagation within the material. The model for the curve was parametrized for Cu and published by Nagy and Echenique. \cite{Nagy2012}

\subsection[photo\_imfp\_value]{\tt real(kind=dp) :: photo\_imfp\_value(1 or \#layers)}

Set the inelastic mean free path (IMFP) value(s) in \AA~for the calculation. See {\tt photo\_imfp\_model} for further explanation. When using {\tt layers} the number of values must equal the number of explicit layers being used for the calculation. This is generally \nicefrac{\# slab layers}{2} for an even number of total layers and \nicefrac{\# slab layers}{2} + 1 for an odd number of total layers. In case the user gives the layer boundaries, the number of layers determined, has to match the number of layers given in {\tt photo\_layers\_tops}.
...
\item[{\bf --}]  \verb#ekin_ptrans_map#\\ \\
Calculate and write a \textit{kinetic energy} vs \textit{transverse momentum} map as a matrix with the value of each grid point representing the QE contribution at that point. Both the energy and the transverse momentum bin widths are controlled by {\tt photo\_pmat\_bin\_width}. \\
...
Calculate and write a {\boldmath$p_x$,$p_y$,$p_z$} tensor of grid points for electron momenta in \nicefrac{1}{\AA}. Each value at that grid point represents the QE contribution at that specific set of electron momenta values. \\The contributions are Gaussian broadened in {\boldmath$p_x$} and {\boldmath$p_y$} with a FWHM of half a step between k grid points, $i.e.$ MP grid points. In the {\boldmath$p_z$} direction the value {\tt photo\_bindenergy \_broadening} is converted to \nicefrac{1}{\AA} to broaden in the {\boldmath$p_z$} direction. The ranges for emission angles $\theta$ and $\phi$ can be set to allow for comparison with ARPES data.\\
This is the largest of the outputs: one number per grid point, typically a few
hundred megabytes of text per photon energy, and one file per step of a
{\tt photo\_energy\_sweep}. The tensor is normalised to its own integrated
contribution rather than to the quantum efficiency, so its sum is not $QE_{tot}$.
\end{itemize}
...
The default value is $0.01285$ eV, half of $k_B \cdot 298$ K.

Note that for the momentum resolved outputs this width sets the bin spacing but
not the effective resolution, which is limited by the width of the broadened
delta function enforcing energy conservation -- see Section
\ref{subsec:momentum_outputs}.

\subsection[photo\_pmat\_bin\_width]{\tt real(kind=dp) :: photo\_pmat\_bin\_width}
...
|  Electric Field Strength    (V/m)          :     0.0000                    |
...
| Effective Work Function    4.5430 eV         Electric Field    0.0000 V/m  |
...
! ATTENTION: E_Fermi must either match the OptaDOS computed E_Fermi or be set to the same value
! ATTENTION: E_workfct must be set to the same value during creation of fem_bin file and photoemission run
