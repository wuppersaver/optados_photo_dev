...
the acceptance of an analyser. The polar window is given directly as
{\tt photo\_theta\_min} and {\tt photo\_theta\_max}, measured from the surface
normal.

The azimuth is given as a direction and a half width, {\tt photo\_phi\_centre}
and {\tt photo\_phi\_halfwidth}, both in degrees and measured from the cartesian
$x$ axis. A pair of bounds cannot describe an analyser centred on the $x$ axis,
which spans $-30^\circ$ to $+30^\circ$ and wraps through $180^\circ$; a centre
and a half width can. The default half width of $180^\circ$ accepts the whole
circle, which is what the earlier keywords {\tt photo\_phi\_min} and
{\tt photo\_phi\_max} did in practice. Those keywords are no longer accepted:
they acted on an azimuth that had been folded into the first quadrant, so a
narrowed window selected one wedge in each of the four quadrants rather than a
single wedge, and reinterpreting an old input file against the signed azimuth
would silently change what it asked for.
