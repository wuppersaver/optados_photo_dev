...
Sets the assumed middle of the slab in the case that the layer boundaries are determined by the user input. When this is set, {\tt photo\_slab\_min} and {\tt photo\_slab\_max} \textbf{cannot} be set and {\tt photo\_layers\_tops} \textbf{must} be set. This value is used to calculate the height of the innermost layer with the last value in the vector of {\tt photo\_layers\_tops}.

There is no default value

\subsection[photo\_layers\_tops]{\tt real(kind=dp) :: photo\_layers\_tops(\#layers)}

This vector sets the assumed top $z$-coordinate for the layers going from the surface towards the middle. Split each value by a space. The first value is inferred to be the slab boundary and each layer that the user wants to include in the calculation needs to be represented in the vector. This makes it possible to calculate the layer heights, layer volumes and atoms in a layer. Some more user input is needed than if {\tt photo\_slab\_min} and {\tt photo\_slab\_max} were set, but there are way less assumptions by the program.
